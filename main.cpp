...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	const uint32_t kNumInstance =2; //インスタンス数
	...
	for (uint32_t index = 0; index < kNumInstance; ++index)
	{
		...
		transforms[index].scale = { 0.4f,0.6f,0.5f };
	...
	rootParameters[3].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV; //CBVえお使う
	...
	const float kLonEvery = (2 * kPi) / float(kSubdivision);//緯度分割1つ文の角度
	const float kLatEvery = kPi / float(kSubdivision);//緯度分割の一つ分の角度
	...
	//materialDataSprite->color = Vector4(1.0f, 0.0f, 0.0f, 0.0f);
	...
	//shaderをコンバートする
	IDxcBlob* vertexShaderBlob = ComileShader(L"Object3d.VS.hlsl",
		L"vs_6_0", dxcUtils, dxcCompiler, includeHandler);
	assert(vertexShaderBlob != nullptr);
	
	IDxcBlob* pixelShaderBlob = ComileShader(L"Object3d.PS.hlsl",
